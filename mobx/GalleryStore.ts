...
export default class GalleryStore {
  ...
  async getBase64Image(url: string, id: string, width: number, height: number) {
    if (this.isAppInternetReachable) {
      try {
        ...
        if (getImageResponse) {
          runInAction(() => {
            this.base64Images[id] = `data:${getImageResponse.headers[
              'content-type'
            ].toLowerCase()};base64,${encode(
              <ArrayBuffer>getImageResponse.data,
            )}`;
          });
        }
      } catch (e) {
    ...
    if (!this.isAppSync) {
      ...
      try {
        ...
        if (this.isAppInternetReachable) {
          ...
          const imagesFromStorage: {[key: string]: string} =
            await readFromStorage(STORAGE_BASE64_IMAGE);
          ...
          const base64UsersAvatarFromStorage: {[key: string]: string} =
            await readFromStorage(STORAGE_USERS_AVATAR);
        ...
        try {
          ...
          if (detailResponseData) {
            ...
            if (!this.base64UsersAvatar[detailResponseData.user.id]) {
              const getImageResponse = await galleryAPI.getImageByUrl(
                detailResponseData.user.profile_image.large,
              );

              if (getImageResponse) {
                runInAction(() => {
                  this.base64UsersAvatar[
                    detailResponseData.user.id
                  ] = `data:${getImageResponse.headers[
                    'content-type'
                  ].toLowerCase()};base64,${encode(getImageResponse.data)}`;
                });
              }
            }
          }
  ...
  setViewableItems(viewableItems: ViewToken[]) {
    runInAction(() => (this.viewableItems = [...viewableItems]));
  }
