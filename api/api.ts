...
export const galleryAPI = {
  async getGallery(currentPage: number, pageSize: number) {
    try {
      return instance.get<PhotoType[]>(`/photos?page=${currentPage}&per_page=${pageSize}`);
    } catch (e) {
  ...
  async getSearchedGallery(searchText: string, currentPage: number, pageSize: number) {
    try {
      return instance.get<{ results: PhotoType[] }>(`/search/photos?query=${searchText}&page=${currentPage}&per_page=${pageSize}`);
    } catch (e) {
