...
export const GalleryRow: React.FC<IProps> = ({row}) => {
  ...
  let imageDimensions = row.map(image =>
    calcImageDimensions(
      appWidth,
      (appWidth * image.height) / image.width,
      row.length,
    ),
  );
  ...
  if (imageDimensions.length > 1) {
    ...
    const imagesWidth = imageDimensions.reduce(
      (sum, el) => sum + el['width'],
      0,
    );
    const imagesHWidth = imageNormalizedHeightDimensions.reduce(
      (sum, el) => sum + el['width'],
      0,
    );
      ...
      {row.map((el, index) => (
        <View
          ...
          <GalleryItem
            ...
            imageDimensions={imageDimensions[index]}
          />
        </View>
      ))}
    </View>
